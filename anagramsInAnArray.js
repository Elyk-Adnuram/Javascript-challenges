...
//other solution
function isAnagramInArray(anagram, arr) {
  //set anagram phrase to lowercase, alphabetical order and remove whitespace
  const revisedAnagram = anagram.toLowerCase().split("").sort().join("").trim();
  //set phrases in array to lowercase, alphabetical order and remove whitespace
  const noSpacesArray = arr.map((phrase) => {
    return phrase.toLowerCase().split("").sort().join("").trim();
  });
  //use filter method to check if anagram exists in the array
  return noSpacesArray.filter((phrase) => {
    return phrase.includes(revisedAnagram);
  });
}
